...
const mockPacket: DebugPacket = {
  issue: { id: 'x', summary: 'Checkout fails after retry' },
  anchors: { feature_slugs: ['checkout'], error_signatures: ['PaymentRetryError'], symptoms: [] },
  relevant_docs: [{ doc_path: 'docs/checkout.md', heading: 'Retry Policy', match_count: 2 }],
  ...
  prior_fixes: [{ fix_id: 'fix:1', summary: 'Reset retry counter', kind: 'code-change', match_count: 2 }],
}
...
it('shows Build Debug Packet button when status is not_run', () => {
  render(<ModokPanel ticketId="ACME-1842" run={notRun} onRun={jest.fn()} />)
...
it('shows Build Debug Packet button when last run failed', () => {
  render(<ModokPanel ticketId="ACME-1842" run={failedRun} onRun={jest.fn()} />)
  expect(screen.getByRole('button', { name: /build debug packet/i })).toBeInTheDocument()
})

// @spec DEMO-MODOK-002, DEMO-MODOK-003
it('shows spinner and disables button while POST is in flight', () => {
  render(<ModokPanel ticketId="ACME-1842" run={notRun} onRun={jest.fn()} isRunning={true} />)
...
it('renders debug packet when POST completes successfully', async () => {
  ...
  const { rerender } = render(<ModokPanel ticketId="ACME-1842" run={notRun} onRun={onRun} />)
  fireEvent.click(screen.getByRole('button', { name: /build debug packet/i }))
  rerender(<ModokPanel ticketId="ACME-1842" run={completeRun} onRun={onRun} />)
  await waitFor(() => {
...
it('displays debug packet on initial load when status is already complete', () => {
  render(<ModokPanel ticketId="ACME-1842" run={completeRun} onRun={jest.fn()} />)
  expect(screen.getByText(/checkout fails after retry/i)).toBeInTheDocument()
})
...
it('omits sections with no items', () => {
  ...
  render(<ModokPanel ticketId="X" run={sparseRun} onRun={jest.fn()} />)
...
it('shows error message for issue_not_found', () => {
  ...
  render(<ModokPanel ticketId="X" run={run} onRun={jest.fn()} />)
...
it('shows partial ingest warning banner alongside the packet', () => {
  ...
  render(<ModokPanel ticketId="ACME-1842" run={partialRun} onRun={jest.fn()} />)
...
it('shows raw stdout in collapsible when error is parse_error', () => {
  ...
  render(<ModokPanel ticketId="X" run={parseErrRun} onRun={jest.fn()} />)

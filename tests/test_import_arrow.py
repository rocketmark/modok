...
def make_repo(tmp_path: Path) -> Path:
    ...
    """
    # index.yaml
    write(tmp_path / "docs/arrows/index.yaml", """\
        arrows:
          - id: widget-core
    ...
    write(tmp_path / "docs/arrows/widget-core.md", """\
        # Arrow: widget-core
    ...
    write(tmp_path / "docs/arrows/util-helper.md", """\
        # Arrow: util-helper
    ...
    code_map = make_code_map([
        ...
        make_code_map_entry("client/util/helper.py",
...
def test_import_arrow_conflict_exits_nonzero_no_output(tmp_path):
    ...
    with patch("modok.import_arrow.dedup.find_duplicate_pairs", return_value=[("renderer", "renderer-b")]):
...
def test_feature_description_verbatim(tmp_path):
    ...
    index = [{"id": "widget-core", "description": desc, "specs": "s.md", "arrow_doc": "a.md", "tests": []}]
    write(tmp_path / "a.md", "### Code\n")
    features = extract_features(index, repo_root=tmp_path)
...
def test_feature_specs_verbatim(tmp_path):
    ...
    index = [{"id": "widget-core", "description": "X", "specs": specs_path, "arrow_doc": "a.md", "tests": []}]
    write(tmp_path / "a.md", "### Code\n")
    features = extract_features(index, repo_root=tmp_path)
...
def test_feature_no_code_section_empty_source_files_warns(tmp_path, capsys):
    ...
    index = [{"id": "feat-a", "description": "X", "specs": "s.md", "arrow_doc": "a.md", "tests": []}]
...
def test_feature_missing_required_field_exits_nonzero(tmp_path):
    ...
    write(tmp_path / "docs/arrows/index.yaml", textwrap.dedent("""\
        arrows:
          - id: feat-a
    ...
    write(tmp_path / ".modok/code-map.yml", yaml.dump(make_code_map([])))
    runner = CliRunner()
    result = runner.invoke(cli, ["import-arrow", "--project", "x", "--repo", str(tmp_path), "--no-llm"])
    assert result.exit_code != 0
...
def test_module_pass1_python_file_per_module():
    ...
    code_map = make_code_map([
        make_code_map_entry("client/widget/renderer.py", symbols=[{"name": "WidgetRenderer", "kind": "class", "line_start": 1}]),
        make_code_map_entry("client/widget/state.py", symbols=[{"name": "WidgetState", "kind": "class", "line_start": 1}]),
    ])
...
def test_module_pass1_cpp_files_same_parent_become_directory_module():
    ...
    file_modules = [s for s, e in modules.items() if "agent/src/agent.c" in e.get("source_files", [])]
...
def test_overlay_warns_and_skips_when_symbol_in_multiple_files(capsys):
    ...
    modules = {
        "renderer-a": {"source_files": ["client/a/renderer.py"], "description": "", "primary_class": "Renderer", "language": "python", "name": "Renderer A", "source_roots": []},
        "renderer-b": {"source_files": ["client/b/renderer.py"], "description": "", "primary_class": "Renderer", "language": "python", "name": "Renderer B", "source_roots": []},
    }
    code_map = make_code_map([
        make_code_map_entry("client/a/renderer.py", symbols=[{"name": "Renderer", "kind": "class", "line_start": 1}]),
        make_code_map_entry("client/b/renderer.py", symbols=[{"name": "Renderer", "kind": "class", "line_start": 1}]),
    ])
    arrow_key_components = {
        "feat-a": [{"class_name": "Renderer", "description": "the renderer"}]
    }
    updated = apply_key_components_overlay(modules, arrow_key_components, code_map)
...
def test_overlay_warns_and_skips_when_symbol_in_multiple_arrow_docs(capsys):
    ...
    modules = {
        "renderer": {"source_files": ["client/renderer.py"], "description": "", "primary_class": "Renderer", "language": "python", "name": "Renderer", "source_roots": []},
    }
    code_map = make_code_map([
        make_code_map_entry("client/renderer.py", symbols=[{"name": "Renderer", "kind": "class", "line_start": 1}]),
    ])
...
def test_overlay_warns_and_skips_when_symbol_not_in_code_map(capsys):
    ...
    modules = {
        "renderer": {"source_files": ["client/renderer.py"], "description": "", "primary_class": "", "language": "python", "name": "Renderer", "source_roots": []},
    }
    code_map = make_code_map([
...
def test_llm_pass1_sends_batched_call():
    ...
    modules_to_name = [
        {"slug": "shtp-receiver", "primary_class": "ShtpReceiver", "source_file": "client/shtp_receiver.py", "language": "python"},
        {"slug": "ltc-service", "primary_class": "LtcTimecodeService", "source_file": "client/ltc.py", "language": "python"},
    ]
    mock_llm = MagicMock(return_value=[
        {"slug": "shtp-receiver", "name": "SHTP Receiver", "description": "Receives SHTP packets"},
        {"slug": "ltc-service", "name": "LTC Timecode Service", "description": "Decodes LTC audio"},
    ])
...
def test_llm_dedup_sends_one_call_per_pair():
    ...
    modules = {
        "renderer-a": {"name": "Renderer A", "description": "D", "source_files": ["client/r.py"], "source_roots": []},
        "renderer-b": {"name": "Renderer B", "description": "D", "source_files": ["client/r.py"], "source_roots": []},
        "writer-a": {"name": "Writer A", "description": "D", "source_files": ["client/w.py"], "source_roots": []},
        "writer-b": {"name": "Writer B", "description": "D", "source_files": ["client/w.py"], "source_roots": []},
    }
    call_args_list = []
...
def test_llm_pass2_failure_treats_as_conflict():
    ...
    pairs = [("renderer-a", "renderer-b")]
    modules = {
        "renderer-a": {"name": "A", "description": "D", "source_files": ["client/r.py"], "source_roots": []},
        "renderer-b": {"name": "B", "description": "D", "source_files": ["client/r.py"], "source_roots": []},
    }
...
def test_output_files_are_valid_yaml(tmp_path):
    ...
    features = {"feat-a": {"name": "Feat A", "description": "X", "source_files": [], "test_files": [], "specs": "s.md", "modules": []}}
    modules = {"mod-a": {"name": "Mod A", "description": "Y", "source_files": [], "source_roots": [], "primary_class": "", "language": "python"}}
    write_registries(features, modules, repo_root=tmp_path)
...
def test_output_both_files_regenerated_together(tmp_path):
    ...
    write_registries(
        {"feat-a": {"name": "A", "description": "X", "source_files": [], "test_files": [], "specs": "s.md", "modules": []}},
        {"mod-a": {"name": "A", "description": "Y", "source_files": [], "source_roots": [], "primary_class": "", "language": "python"}},
        repo_root=tmp_path,
    )

    write_registries(
        {"feat-b": {"name": "B", "description": "Y", "source_files": [], "test_files": [], "specs": "s.md", "modules": []}},
        {"mod-b": {"name": "B", "description": "Z", "source_files": [], "source_roots": [], "primary_class": "", "language": "python"}},
        repo_root=tmp_path,
    )

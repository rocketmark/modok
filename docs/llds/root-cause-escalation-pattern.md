...
**`status` is never trusted for the append-vs-new decision — GitHub is, always checked live.** Unlike `FileEscalation` (where "does this escalation already exist" is the only state that matters), this component must know whether the escalation's issue is *currently open on GitHub*, which can change without MODOK ever writing to Quine (a human closing the issue). Every decision that depends on open/closed state calls the GitHub API directly (§ `_process_root_cause_escalation` below) — `status` is never read for that purpose, and a stale `status` value can never cause a wrong append-vs-new decision.

**`status` *is* kept in sync for browsing/querying, found necessary live — reversing this LLD's original "decorative only, never synced" design.** A user closed a real escalation issue, then queried the graph (Quine's own Graph Explorer UI) expecting to see that reflected, and found the `RootCauseEscalation` node still reporting `status: "open"` — because nothing updated it outside the lazy, decision-time-only check. The original rationale (syncing risks going stale between checks, so don't bother) was correct about the *decision* path but missed that `status` is also the only field a human has to inspect current state by — leaving it permanently `"open"` isn't merely uninformative, it's actively wrong the moment an issue closes. `reconcile_root_cause_escalation_status` (§ below) closes this: a periodic sweep, independent of the append-vs-new decision path, that syncs `status` to `"closed"` once GitHub confirms it — still never consulted by the decision logic itself, so the original correctness argument for that path is untouched.
...
**Found live, after this component shipped — not designed speculatively.** A user closed a real escalation issue on GitHub, then queried the graph directly and found `RootCauseEscalation.status` still `"open"`, since nothing synced it outside the lazy, decision-time-only check in `_process_root_cause_escalation`. `status` was deliberately decorative in the original design (§ Graph Model); this sweep exists purely for browsing/querying accuracy, and is a completely independent code path from the append-vs-new decision logic — nothing about the decision path's correctness argument changes.

`reconcile_root_cause_escalation_status(client: Any, project_slug: str, github_repo: str, token: str) -> int` (`src/modok/ingestion/ci_ingestion.py`, alongside the other two `RootCauseEscalation`-related functions):

1. Query every `RootCauseEscalation` for the project where `status = 'open'` and `github_issue_number <> ''` (a pending/never-created escalation has no real issue to check yet — skipped, not an error).
2. For each, call `get_issue_state(github_repo, token, github_issue_number)` (§ GitHub Issue State — the same function `_process_root_cause_escalation` already uses for the decision path; this is the only thing the two code paths share).
3. WHEN the result is `"closed"`: `SET status = 'closed'` via the same targeted-property-update technique used elsewhere in this component (`client.query()` with `idFrom()` embedded).
4. WHEN the result is `"open"` or `None` (transient API failure): no write. `None` is retried next cycle; `"open"` correctly stays `"open"` — not distinguished from each other in the write path, since both mean "don't change anything."

**Once `status` is `"closed"`, it is never re-checked or reverted.** Mirrors `GHING-DEL-007`'s identical reasoning for deleted tickets: a `RootCauseEscalation`'s `sequence` scheme already treats "this window is closed" as a permanent, one-way fact driving the *next* window's creation — a status-sync sweep re-opening that same conclusion for browsing purposes would contradict the decision path's own semantics, and GitHub issues reopening after being programmatically observed as closed is not a case this component needs to handle (a human reopening one is equivalent, in effect, to filing a new ticket that happens to reference an old issue — not something MODOK tracks either way).

**Runs on the same poll cycle as `reconcile_root_cause_escalations`**, wired into `_run_ci_ingestion_cycle` in its own `try`/`except`, cost-bounded by the number of currently-`"open"`-marked escalations (not ticket volume) — the same bound `_process_root_cause_escalation`'s own live check already carries, just applied proactively instead of only at threshold-crossing time.
...
- `RootCauseEscalation.status` is never *read* for the append-vs-new decision — GitHub's live issue state is authoritative for that, not the graph. (It *is* written, proactively, by `reconcile_root_cause_escalation_status` — § Status Sync — for browsing/querying accuracy; that write path is independent of, and never consulted by, the decision path.)
...
| `RootCauseEscalation.status` sync | A dedicated periodic sweep (`reconcile_root_cause_escalation_status`), independent of the decision path | Sync `status` inline whenever `_process_root_cause_escalation`'s own live check happens to run; or leave `status` permanently decorative (the original design) | User-selected, found live: leaving `status` permanently `"open"` was actively wrong the moment a real issue closed, not merely uninformative — a human has no other field to check current state by. Inline-only sync was rejected because `_process_root_cause_escalation` only runs when a *new* threshold-crossing event happens for that feature, which could be arbitrarily long after the issue actually closed — a dedicated sweep checks every open-marked escalation every cycle regardless of ticket activity, closing that gap. This sync is still never read by the decision path itself, so the original staleness-during-decision concern is fully preserved |

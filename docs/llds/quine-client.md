...
**A second, deeper bug surfaced while live-testing the fix above.** Fixing the ID *type* mismatch wasn't enough — `node_exists`/`node_exists_by_parts` still returned `True` for addresses that were never written to at all. Directly probing a brand-new, never-before-referenced address twice in a row (`MATCH (n) WHERE id(n) = idFrom('feature', 'stagehand', 'totally-made-up-slug-xyz-123') RETURN n`) returned an identical `{"properties": {}}` row both times — proof this is Quine's addressing model itself (every possible `idFrom()` output is an addressable node, existent or not), not a first-touch vivification side effect. `return bool(results)` can never distinguish "a real node" from this always-present empty shell, so `node_exists`/`node_exists_by_parts` had never actually returned `False` for anything — the "never invent a node" guarantee mechanical anchor linking, `ingest_pr`'s edge gates, and the `Investigation` dedup check all depend on was not being enforced at all. Concretely, this is exactly why a real bug-labeled `CustomerIssue` written during live testing produced a `PostToEndpoint` delivery (`200 OK`, confirmed via the standing query's own match-count stat) but no `Investigation` node: `_process_investigation`'s dedup check queried the not-yet-existing `Investigation` address, got back an empty shell, and — under the old `bool(results)` logic — concluded "already recorded" and skipped the real upsert entirely, on the very first delivery.

Fixed by requiring a `node_type` property on the returned node (`_row_is_real_node`, `src/modok/quine/client.py`) — the exact same discipline `collect_nodes()` already applies for `modok recall` (`CLI-REC-009`). No call site changed; both methods' callers already just trust the boolean.

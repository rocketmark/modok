...
`quick_investigation_summary(issue_id, project_slug, client, feature_source_files=None, backend="local") -> str` is a separate, fast entry point used only by the GitHub write-back's immediate "triggered" comment (`docs/llds/standing-queries.md § Two comments per investigation`) — not part of `retrieve()`'s own call graph. It exists specifically to be cheap: it does none of `retrieve()`'s traversal (feature/module-to-file, error-to-known-issue, fix, similarity, or commit lookups) and no evidence scoring, so it returns in roughly one LLM call's worth of time instead of several traversal round-trips plus a decay/scoring pass plus the summary call.

Steps:
1. Fetch the `CustomerIssue` node. On failure, return `""`.
2. Fetch graph-first anchors via the same `_graph_anchors` helper `retrieve()` uses (`AFFECTS`/`HAS_ERROR` edges — DRE-ANCH-001/002). No LLM fallback is attempted here even if this comes back empty; that's `retrieve()`'s job for the slower, full analysis.
3. For each resolved feature slug, look up `feature_source_files.get(slug, [])` (the registry's declared primary files for that feature — the same field `DRE-TRAV-009`'s primary/peripheral split uses) and union them into a `relevant_files` list. No graph traversal is involved — this is a pure registry lookup.
4. Call `gateway.summarise_packet` with `issue_text` (raw ticket text, or its summary/title), the resolved `module_slugs`/`error_signatures`, and the registry-derived `relevant_files` — but `symptoms`, `relevant_tests`, `matched_elements`, `recent_commits`, and `known_issues` all empty. Reusing this same function (rather than a dedicated prompt) keeps the quick and full summaries stylistically consistent — both describe "what's likely wrong and where" in the same voice, just from different amounts of evidence.
5. On any failure past step 1 (anchor query, or the LLM call itself), fall back to `CustomerIssue.summary` (the ticket title) — never raise.

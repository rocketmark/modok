...
`quick_investigation_summary(issue_id, project_slug, client, feature_source_files=None) -> str` is a separate, fast entry point used only by the GitHub write-back's immediate "triggered" comment (`docs/llds/standing-queries.md § Two comments per investigation`) — not part of `retrieve()`'s own call graph. It exists specifically to be cheap: no traversal (feature/module-to-file, error-to-known-issue, fix, similarity, or commit lookups), no evidence scoring, and **no LLM call** — so it returns in about the time of a couple of Quine round-trips.

**No LLM call, by design, after a live finding.** The first version of this function reused `gateway.summarise_packet` with a reduced input, on the theory that skipping traversal alone would make it "fast enough." Measured standalone, that call took ~85 seconds — an LLM call, even a small one, is tens of seconds to a few minutes on local inference regardless of how little context it's given. Worse, in production the gap between this comment and the "results" comment right after was often just a few seconds, not the intended head start: `retrieve()`'s own summary call (`DRE-SUMM-001`) ran moments later and landed on an already-warm local model, finishing quickly, while *this* function's call had just paid the full cold-start cost. The fix wasn't a faster prompt — it was removing the LLM call from this path entirely, moving all natural-language generation to the one summary that's supposed to take a while.

Steps:
1. Fetch the `CustomerIssue` node. On failure, return `""`.
2. Fetch graph-first anchors via the same `_graph_anchors` helper `retrieve()` uses (`AFFECTS`/`HAS_ERROR` edges — DRE-ANCH-001/002). No LLM fallback is attempted here even if this comes back empty; that's `retrieve()`'s job for the slower, full analysis. If both come back empty, return `CustomerIssue.summary` (the ticket title) immediately.
3. For each resolved feature slug, look up `feature_source_files.get(slug, [])` (the registry's declared primary files for that feature — the same field `DRE-TRAV-009`'s primary/peripheral split uses) and union them into a deduplicated, first-seen-order list, capped at 5.
4. Mechanically compose the summary: `"Features: {slugs}"` and/or `"Errors: {sigs}"` (joined by ` · `), plus `". Likely files: {paths}"` if any were found. No template beyond this string join — deliberately not natural language, since natural language here is exactly what required the LLM call being removed.
5. On any failure past step 1 (the anchor query itself), fall back to `CustomerIssue.summary`.

This function never raises under any failure mode; the caller (`_maybe_notify_github`) still wraps the call defensively, but the function's own contract is "always return a string, even if empty."
...
4. Files from step 1 or 3 get `feature_anchor` evidence (score 8.0). Test files reached via `HAS_TEST` do **not** get a scored evidence item — see § Test Coverage (Informational), below, for why and what they get instead.
...
**Bare `HAS_TEST` coverage is not scored evidence.** Found live against a real GitHub issue (rocketmark/stagehand#31): a test file covering two features stacked one `test_coverage` hit per feature via the same-type geometric decay (§ Candidate Scoring) — `7.0 + 7.0×0.5 = 10.5` — which alone crossed the MEDIUM confidence threshold for a file with zero evidence actually tying it to that specific ticket. "This test happens to cover an area the ticket touches" and "this test is evidence for *this* ticket" are different claims; the old design conflated them.

Every test path reached via `HAS_TEST` for a resolved anchor slug is recorded in `covered_tests_map` (`path -> [covering_slugs]`, deduplicated so a test covered by *N* features is one entry with *N* slugs, not *N* stacked evidence items) and still seeded as an eligible (empty-evidence) key in the test-file evidence map — eligible, not scored, so it can still accrue real evidence from later steps (recent commits, `commit_message_match`, `function_anchor_match`, `ticket_mention`) exactly as before.

**After all evidence-gathering completes**, immediately before scored candidates are built: for every path in `covered_tests_map`, if its evidence map entry is still empty, it is removed from the scored-candidate evidence map and added to `DebugPacket.covered_tests` (a list of `{path, covering_slugs}`) instead. A test file that *did* accrue real evidence elsewhere keeps its place in `scored_candidates`/`relevant_tests` and is not also duplicated into `covered_tests` — the two lists are disjoint by construction.

This directly answers the "what do I actually want to know about test files" question the live finding raised: "does a test exist for this area" is informational context (`covered_tests`), always worth showing but never worth ranking; "this specific test is implicated by this ticket" is a scored claim, earned the same way any other file earns it. Tying `covered_tests` to recent CI failure state (a test that both covers the area *and* failed recently is much stronger evidence than either fact alone) is a deliberate follow-up, not done here — it requires linking `TestFile` to `TestExecution`/`TestFailure`, which does not exist yet (see Open Questions).
...
For each commit, its own `message` (first line, tokenized via `extract_text_tokens`) is checked against `anchor_tokens`. A match adds `commit_message_match` evidence (9.0) to every file the commit touched, in addition to that file's flat `recent_commit` evidence — see § Candidate Scoring. This is a distinct, independent signal from `function_anchor_match` (which inspects the commit's diff content for a matching symbol) — a commit can match by message alone (its content might not touch the relevant symbol at all, e.g. a wifi-package-install fix in an OS image build script) or by diff alone, or both.
...
| `commit_message_match` | 9.0 | The commit's own message shares a token with the ticket's anchors (feature/module slugs, error signatures, symptoms) |
| `doc_penalty` | negative | Applied to non-source files (×0.25 actionability multiplier) |

Bare test coverage (`Feature`/`Module -[:HAS_TEST]-> TestFile` with no other evidence) is **not** in this table — it is informational, reported via `DebugPacket.covered_tests`, not a scored `EvidenceItem` type. See § Test Coverage (Informational).
...
3. A diversity bonus of `3.0 × min(unique_types - 1, 4)` is added — but only counting types at all if the candidate has at least one type outside `_NON_CORROBORATING_TYPES` (`recent_commit`, and peripheral `feature_anchor`). If a candidate's *only* evidence is non-corroborating (e.g. a broad `feature_anchor` match plus a string of unrelated recent commits), the bonus is 0 — those weak signals can't manufacture apparent strength together. But once a candidate has at least one genuinely direct evidence type (`element_anchor_match`, `function_anchor_match`, `commit_message_match`, `ticket_mention`, `feature_primary_file`, ...), *all* of its evidence types count toward the bonus, including the non-corroborating ones — recency reinforcing an already-plausible candidate is real corroboration; recency propping up an otherwise-unevidenced one is not. Found live in both directions — see `DRE-CAND-006`'s history for the two live cases (a peripheral file with no other evidence wrongly getting the bonus; a well-anchored file wrongly losing it).
...
| `recent_commit` / peripheral `feature_anchor` excluded from diversity bonus, conditionally | Excluded only when they're the candidate's *only* evidence type(s); counted when at least one direct type is also present | Unconditional exclusion (tried first); a third weight tier between "counts" and "doesn't" | Unconditional exclusion fixed the peripheral-file-with-only-weak-evidence case but wrongly discounted recency for candidates that were already well-evidenced by direct means (element/function match), regressing a real ticket's score well below MODOK v1's for the same input. Conditioning on "does this candidate have direct evidence at all" fixes both without a new weight dimension |
| `commit_message_match` evidence type | New type: commit's own message token-overlaps `anchor_tokens`, score 9.0, corroborating | Only rely on `function_anchor_match` (diff-content matching) | A commit can be topically relevant to a ticket without its diff touching a matched symbol — found live: a commit titled "fixed wifi provisioning" only touched OS-image build scripts, not the application-level wifi logic, but its message was a much stronger signal than `recent_commit` gave it credit for |
...
| Bare test coverage demoted to informational | `covered_tests` field, not a scored `EvidenceItem`; test files stay eligible for real evidence from later steps | Cap/dedupe `test_coverage` to one hit per test file, keep it scored at a lower weight | A capped score still ranks a structurally-covering-but-otherwise-unimplicated test above files with zero evidence at all, which is the same category error at a smaller magnitude — the live finding (rocketmark/stagehand#31) was about a category confusion ("covers this area" read as "implicated by this ticket"), not a miscalibrated weight, so a weight change wouldn't have fixed the actual complaint |
...
7. **Link `covered_tests` to recent CI failure state** — a test that both covers the ticket's area *and* recently failed in CI is materially stronger evidence than either fact alone (raised alongside the `covered_tests` demotion, rocketmark/stagehand#31). Requires linking `TestFile` (keyed by `repo_path`) to `TestExecution`/`TestFailure` (keyed by `run_id`/`run_attempt`/`classname`/`test_name`, from `docs/llds/continuous-ci-ingestion.md`) — no such edge exists today; CI ingestion and the code map currently have no path from a JUnit `classname`/`test_name` back to a file path. Deliberately scoped as separate follow-up work, not part of this demotion.

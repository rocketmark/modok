...
`format_debug_packet_markdown(packet: DebugPacket, investigation_id: str, standing_query_name: str) -> str` renders the **full** packet — all of the same underlying fields `ui/src/components/modok/DebugPacketView.tsx` shows in the demo app (anchors, affected areas, scored candidates, recent commits), not the thinner subset `diagnose.py`'s `_print_packet` prints to a terminal. `retrieve()` already computes all of this regardless of caller; the original version of this formatter silently dropped it, discarding real evidence that was already sitting on the packet — found live when a real, over-matched ticket's comment showed only a flat file list with no way to distinguish a strong match from a weak one. The two have since diverged in presentation, not data: this formatter's doc/plain-test grouping (SQ-GH-006/008) and per-commit evidence grouping (SQ-GH-009) are GitHub-comment-specific rendering choices with no equivalent in the demo UI, which is a separate, lower-priority surface not required to track this formatter's presentation changes.

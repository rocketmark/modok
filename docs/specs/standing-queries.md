...
- [x] **SQ-GH-010** [U]: The commit group header (`SQ-GH-009`) shall include the commit's date as `{sha} ({date})` when a matching entry (by 7-character SHA prefix) exists in `packet.recent_commits`; the date shall be omitted (header reads just `{sha}:`) when no match is found. This is a formatter-level lookup against data already present on the packet — no new field on `EvidenceItem` is needed since `packet.recent_commits` already carries each commit's `timestamp`.

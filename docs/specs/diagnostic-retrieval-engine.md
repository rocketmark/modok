...
- [x] **DRE-CAND-006** [U]: `recent_commit` and `feature_anchor` (the peripheral, non-primary variant introduced by `DRE-TRAV-009`) shall not count toward the number of unique evidence types used to compute the diversity bonus in `DRE-CAND-001`; both still contribute their own decayed score. Found live: even after lowering `recent_commit`'s weight (`DRE-CAND-005`), a file with only a broad `feature_anchor` match plus a handful of unrelated recent commits still received the full +3.0 corroboration bonus just for having a second evidence *type* present, regardless of how weak or uncorrelated that type's evidence was — letting a frequently-edited-but-unrelated operational script (`pi-image/chroot-customize.sh`, 5 unrelated commits) outrank a file directly relevant to the ticket but with no recent commits. Per `docs/scoring-brainstorm.md`, corroboration should reward genuinely independent evidence, not the mere presence of a weak, broad, or uncorrelated signal.
